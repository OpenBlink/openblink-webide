(() => {
  ...
  var TreeNode = class _TreeNode extends BaseNode {
    ...
    nextChild(i, dir, pos, side, mode = 0) {
      for (let parent2 = this; ; ) {
        for (let { children, positions } = parent2._tree, e = dir > 0 ? children.length : -1; i != e; i += dir) {
          let next = children[i], start = positions[i] + parent2.from, mounted;
          ...
          if (next instanceof TreeBuffer) {
            ...
            if (!(mode & IterMode.IgnoreMounts) && (mounted2 = MountedTree.get(next)) && !mounted2.overlay)
              return new _TreeNode(mounted2.tree, start, i, parent2);
            let inner = new _TreeNode(next, start, i, parent2);
    ...
    */
    ...
    atLastNode(dir) {
      ...
      for (; parent2; { index, _parent: parent2 } = parent2) {
        if (index > -1)
          for (let i = index + dir, e = dir < 0 ? -1 : parent2._tree.children.length; i != e; i += dir) {
  ...
  function scrollIntoView2(container, element) {
    ...
    if (self.top < parent2.top)
      container.scrollTop -= (parent2.top - self.top) / scaleY;
    else if (self.bottom > parent2.bottom)
      container.scrollTop += (self.bottom - parent2.bottom) / scaleY;
  }
  ...
  // src/codemirror/editor.mjs
  var parent = document.getElementById("ruby-editor");
  if (!parent) {
    throw new Error("Ruby editor mount point was not found");
  }
  var notifyDocumentChanged = EditorView.updateListener.of((update) => {
    if (update.docChanged) {
